...
def get_data(
    ...
    # Clean outliers
    df.drop(df[(df['temperature'] < cut_off_min) | (df['temperature'] > cut_off_max)].index, inplace=True)
    ...
    df.drop(df[(df['temperature'] < lower_bound) | (df['temperature'] > upper_bound)].index, inplace=True)
    ...
    x_train = reshape_data(x_train, sequence_length)
    x_test = reshape_data(x_test, sequence_length)
